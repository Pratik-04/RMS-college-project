...
// Must be logged in for ordering (but can browse without login)
...
// Handle Add to Cart → redirects to mycart.php
...
// Handle Place Order → redirects to payment.php
$orderMessage = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
    ...
    } elseif (!in_array($city, ['Kathmandu', 'Lalitpur', 'Bhaktapur'])) {
        $orderMessage = "Please select a valid city!";
    } elseif (strlen($street) < 5) {
        $orderMessage = "Please enter your street / detailed location (at least 5 characters)!";
    } else {
        // Combine both into one variable (same format as checkout page)
        $full_location = $street . ', ' . $city;

        $_SESSION['pending_single_order'] = [
            'item'     => $item,
            'quantity' => $quantity,
            'location' => htmlspecialchars($full_location)  // same key & format as pending_checkout
        ];
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  input, select { width: 100%; padding: 14px; margin-top: 8px; border-radius: 10px; border: 1px solid #bbb; font-size: 18px; }
  select { 
      appearance: none; 
  ...
  button#main-order-btn, button#add-to-cart-btn { margin-top: 20px; padding: 16px 40px; background: #2f4f2f; color: white; border: none; border-radius: 30px; font-size: 24px; width: 100%; cursor: pointer; transition: background 0.3s; }
  button#main-order-btn:hover, button#add-to-cart-btn:hover { background: #3e5e3e; }
  button#add-to-cart-btn { background: #ffcc00; margin-top: 15px; }
...
<div class="container">
    <?php if ($showLoginMessage): ?>
        <div class="login-message">
            ...
            <p>You need to be signed in to place an order or add to cart.</p>
    ...
    <?php else: ?>
        ...
        <!-- Delivery fields - now inside the form -->
        <form method="post" id="placeOrderForm">
            ...
            <input type="text" id="streetInput" name="street" placeholder="e.g. Thamel, Lazimpat-16, near Garden of Dreams" required>

            <button type="submit" id="main-order-btn">Place Order Now</button>
        </form>
...
function updateTotal() {
    ...
    document.getElementById("order-qty").value = quantity;
...
document.getElementById("placeOrderForm").addEventListener('submit', function(e) {
    ...
    if (!city) {
        alert("Please select your city (Kathmandu, Lalitpur, or Bhaktapur)");
        e.preventDefault();
        return;
    }
