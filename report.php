...
<script>
    ...
    function generateReport(type) {
        ...
        orders.forEach(order => {
            ...
            if (type === 'daily') {
                const sel = new Date(dailyInput);
                match = orderDate.toDateString() === sel.toDateString();
            } else {
                const [year, month] = monthlyInput.split('-');
                match = orderDate.getFullYear() == year && (orderDate.getMonth() + 1) == month;
            }
            ...
            if (match) {
                ...
                itemsMap[order.item_name] = (itemsMap[order.item_name] || 0) + qty;
        ...
        if (totalItems === 0) {
            resultDiv.innerHTML = `<i>No completed orders found for this period.</i>`;
            return;
        }

        resultDiv.innerHTML = `
            <div style="font-size:26px;color:#27ae60;margin-bottom:18px;">
                <b>${type === 'daily' ? 'Daily' : 'Monthly'} Completed Sales</b>
            </div>
            <p>Total Revenue: <span class="highlight">Rs ${totalRevenue.toLocaleString()}</span></p>
            <p>Total Items Sold: <b>${totalItems}</b></p>
            <p>Top Delivery Location: <span class="location-tag">${topLocation} (${maxCount} items)</span></p>
            ${topItemsList ? `<div class="top-items"><b>Top Selling Items:</b><ul>${topItemsList}</ul></div>` : ''}
        `;
    }
</script>
